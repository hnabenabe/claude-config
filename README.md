...
- スキルをゼロから作成、または既存スキルを編集・最適化
- evalテストの実行とベンチマーク測定
- スキルのdescription最適化によるトリガー精度向上
- パッケージング・配布

**`mcp-builder`** — MCPサーバー構築ガイド：
- Python (FastMCP) / TypeScript (MCP SDK) 対応
- API調査からツール設計、実装、テスト、評価までの一貫したワークフロー
- ベストプラクティス・リファレンス付き

**`webapp-testing`** — Webアプリテスト用ツールキット：
- Playwright を使ったローカルWebアプリのテスト・操作
- サーバーライフサイクル管理スクリプト付き
- スクリーンショット撮影、コンソールログ取得、UI操作の自動化
...
5. `skills/` 内の各スキルディレクトリを `~/.claude/skills/` にコピー
